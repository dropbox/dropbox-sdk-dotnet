...
namespace Dropbox.Api.Files
{
    ...
    /// constitutes a conflict and what the autorename strategy is. In some situations, the
    /// conflict behavior is identical: (a) If the target path doesn't refer to anything, the
    /// file is always written; no conflict. (b) If the target path refers to a folder, it's
    /// always a conflict. (c) If the target path refers to a file with identical contents,
    /// nothing gets written; no conflict. The conflict checking differs in the case where
    /// there's a file at the target path with contents different from the contents you're
    /// trying to write.</para>
    /// </summary>
